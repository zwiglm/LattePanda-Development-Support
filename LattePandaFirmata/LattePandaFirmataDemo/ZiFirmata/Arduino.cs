...
namespace ZiFirmata
{
    ...
    public delegate void DidStringReceive(StringResult stringResult);
    ...
    public class Arduino
    {
        ...
        public event DidStringReceive didStringReceive;
        ...
        public void Open(bool isListen) {
            ...
            // zwim todo: not needed atm - comment
            //for (int i = 0; i < 6; i++) {
                        ...
                        if (inputData == Arduino.END_SYSEX) {
                            ...
                            if (_sysexBytesRead > 5 &&_storedInputData[0] == Arduino.SYSEX_SUB_STRING) {
                                ...
                                for (int i = 0; i < stringReceive.Count(); i++) {
                                    stringReceive[i] = (byte)(_storedInputData[(i * 2) + 1] | _storedInputData[(i * 2) + 2] << 7);
                                }
                                string stringResult = System.Text.Encoding.UTF8.GetString(stringReceive, 0, stringReceive.Length);
                                StringResult result = new StringResult(stringResult);
                                _arduino.callDidStringReceive(result);
                            }
                            _sysexBytesRead = 0;
                        }
                        else {
